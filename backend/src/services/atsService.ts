...
const summarizeStructuredItems = (value: unknown): string => {
  if (!Array.isArray(value)) return '';
  return value
    .map((item: any) => {
      if (!item || typeof item !== 'object') return String(item || '');
      return [
        item.title || item.role || item.position || item.degree || '',
...
async function getAtsResult(jobDescription: string, candidateProfile: string): Promise<AtsResult> {
  ...
  let semanticScore = 0;
  try {
    if (!JINA_API_KEY) throw new Error('Jina API key is not configured');
...
export async function analyseWithAts(jobId: number, userId: number): Promise<AtsResult> {
  ...
  const jobRequiredSkills = parseList((job as any).requiredSkills);
  const jobTechStack = parseList((job as any).techStack);
  const jobDescription = [
    (job as any).title ? `Title: ${(job as any).title}` : '',
    (job as any).description || '',
    jobRequiredSkills.length ? `Required skills: ${jobRequiredSkills.join(', ')}` : '',
    jobTechStack.length ? `Tech stack: ${jobTechStack.join(', ')}` : '',
    (job as any).experienceLevel ? `Experience level: ${(job as any).experienceLevel}` : '',
  ].filter(Boolean).join('\n');

  const profileSkills = parseList((profile as any).skills);
  const githubSkills = parseList((profile as any).githubVerifiedSkills);
  const candidateProfile = `
    Headline: ${(profile as any).headline || ''}
    Skills: ${Array.from(new Set([...profileSkills, ...githubSkills])).join(', ')}
    Experience: ${summarizeStructuredItems((profile as any).experience)}
    Education: ${summarizeStructuredItems((profile as any).education)}
    Projects: ${summarizeStructuredItems((profile as any).projects)}
    Bio: ${(profile as any).bio || ''}
    Resume report: ${JSON.stringify((profile as any).resumeReport || {})}
  `;
