...
const BASE_CANDIDATES = isLocal
  ? [primaryBase, ...(fallbackBase ? [fallbackBase] : [])]
  : ['', primaryBase]; // If on ngrok/external, use relative path first to trigger Next.js rewrites
