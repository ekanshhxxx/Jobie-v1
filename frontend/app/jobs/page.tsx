  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold dark:text-white text-gray-900">Jobs</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{jobs.length} open positions</p>
        </div>
        {user?.role === 'recruiter' && (
          <button
      ...
      {/* Search */}
      <input
        className="w-full border border-gray-200 dark:border-white/10 dark:bg-white/5 dark:text-white rounded-xl px-4 py-2.5 text-sm mb-6 outline-none focus:ring-2 focus:ring-indigo-500"
      ...
      {/* Post Job Form */}
      {showPostForm && (
        <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-6 mb-8 shadow-sm">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-5">Post a New Job</h2>
          <form onSubmit={postJob} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={f.k}>
                ...
                <input
                  className="w-full border border-gray-200 dark:border-white/10 dark:bg-white/5 dark:text-white rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500"
            ...
            <div className="col-span-2">
              ...
              <textarea
                className="w-full border border-gray-200 dark:border-white/10 dark:bg-white/5 dark:text-white rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 h-24 resize-none"
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {filtered.map((job) => (
            <Link href={`/jobs/${job.id}`} key={job.id} className="block bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-indigo-500 dark:hover:border-indigo-400 transition">
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2.5 flex-wrap mb-1">
                    <h3 className="font-semibold text-gray-900 dark:text-white truncate">{job.title}</h3>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${levelColors[job.experienceLevel] ?? 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}`}>
                      {job.experienceLevel}
                    </span>
                  </div>
