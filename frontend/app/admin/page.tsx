...
const LIGHT = {
  ...
  chartBg:'#ececec',
  chartAxis: '#333333',
  chartGridStroke: '#cccccc',
  tooltipBg: '#ffffff',
  tooltipBorder: '#1a1a1a',
};

type Theme = typeof DARK;
const FONT = 'var(--font-jetbrains), "JetBrains Mono", "Fira Code", monospace';
...
function makeChartTip(T: Theme) {
  ...
  return function ChartTip(props: any) {
    ...
    return (
      <div style={{ background: T.tooltipBg, border: `1px solid ${T.tooltipBorder}`, padding: '8px 12px', fontFamily: FONT, fontSize: '11px' }}>
...
function Pane({ title, children, T, action }: { title: string; children: React.ReactNode; T: Theme; action?: React.ReactNode }) {
  return (
    <div style={{ border: `1px solid ${T.border}`, fontFamily: FONT, backgroundColor: T.bg, width: '100%' }}>
      <div style={{ backgroundColor: T.muted, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 12px' }}>
        <span style={{ color: T.green, fontSize: '11px', letterSpacing: '0.06em', textShadow: T.gGlow }}>
          +─ {title.toUpperCase()} ──+
        </span>
        {action}
      </div>
...
function StatBox({ label, value, sub, color, glow, T }: { label: string; value: string | number; sub?: string; color?: string; glow?: string; T: Theme }) {
  ...
  return (
    <div style={{ border: `1px solid ${T.border}`, padding: '20px 24px', backgroundColor: T.bg, fontFamily: FONT, flex: '1 1 140px' }}>
      <div style={{ fontSize: '11px', color: T.muted, letterSpacing: '0.09em', marginBottom: '8px', textTransform: 'uppercase' }}>{label}</div>
      <div style={{ fontSize: '32px', fontWeight: 700, color: c, textShadow: g, lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ fontSize: '10px', color: T.muted, marginTop: '6px' }}>{sub}</div>}
    </div>
  );
}
...
function Badge({ status, T }: { status: string; T: Theme }) {
  ...
  return (
    <span style={{ border: `1px solid ${s.c}`, color: s.c, textShadow: s.g, fontSize: '10px', padding: '1px 7px', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>
      [{status.toUpperCase()}]
    </span>
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        fontFamily: FONT, fontSize: '11px', padding: '3px 10px',
    ...
    <tr style={{ borderBottom: `1px solid ${T.border}` }}>
      {cells.map((c, i) => (
        <td key={i} style={{
          ...
          fontWeight: head ? 700 : 400, letterSpacing: head ? '0.08em' : '0.02em',
...
function ChartPane({ title, children, T }: { title: string; children: React.ReactNode; T: Theme }) {
  return (
    <div style={{ border: `1px solid ${T.border}`, backgroundColor: T.chartBg, fontFamily: FONT }}>
      <div style={{ borderBottom: `1px solid ${T.border}`, padding: '5px 12px', fontSize: '10px', color: T.muted, letterSpacing: '0.08em' }}>
  ...
  return (
    <div style={{ backgroundColor: T.bg, minHeight: '100vh', fontFamily: FONT, color: T.text, position: 'relative' }}>
          ...
          {toasts.map(t => {
            ...
            return (
              <div key={t.id} style={{
                ...
                fontSize: '12px', color: tColor, textShadow: tGlow, letterSpacing: '0.05em',
      ...
      <div style={{ borderBottom: `1px solid ${T.border}`, backgroundColor: T.bg2 }}>
        <div style={{ maxWidth: '1500px', margin: '0 auto', padding: '12px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
            <div>
              <div style={{ fontSize: '18px', fontWeight: 700, color: T.green, textShadow: T.gGlow, letterSpacing: '0.06em' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '13px', color: T.green, textShadow: T.gGlow, fontWeight: 600 }}>{clock}</div>
                <div style={{ fontSize: '10px', color: T.muted, letterSpacing: '0.05em' }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontFamily: FONT, fontSize: '11px', padding: '4px 12px', cursor: 'pointer', letterSpacing: '0.05em',
                }}
              >
                [REFRESH]
              </button>
      ...
      <div style={{ maxWidth: '1500px', margin: '0 auto', padding: '20px 24px' }}>
                    ...
                    {filteredUsers.map(user => (
                      <TRow T={T} key={user.id} cells={[
                        ...
                        user.banned
                          ...
                          : <span key="b" style={{ fontSize: '10px', color: T.muted, letterSpacing: '0.05em' }}>active</span>,
                        fmt(user.createdAt),
        ...
        <div style={{ marginTop: '24px', border: `1px solid ${T.border}`, fontFamily: FONT, backgroundColor: T.bg }}>
          <div style={{ backgroundColor: T.muted, padding: '4px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ color: T.green, fontSize: '11px', letterSpacing: '0.06em', textShadow: T.gGlow }}>
              +─ TERMINAL / ACTIVITY LOG ──+
            </span>
            <span style={{ color: T.green, fontSize: '10px', opacity: 0.6 }}>↑↓ history · enter to run</span>
          </div>
            ...
            {(['all', 'user', 'job', 'app', 'system', 'errors'] as const).map(cat => {
              ...
              return (
                <button key={cat} onClick={() => setActLogTab(cat)} style={{
                  fontFamily: FONT, fontSize: '10px', padding: '6px 16px', cursor: 'pointer',
          ...
          >
            {(actLogTab === 'all' ? cmdLog
              ...
              <div key={i} style={{
                ...
                fontFamily: FONT, letterSpacing: '0.03em', lineHeight: '1.6',
              }}>
                {line.text}
              </div>
            ))}
          ...
          <div
            style={{ borderTop: `1px solid ${T.border}`, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'text' }}
            onClick={() => inputRef.current?.focus()}
          >
            <span style={{ color: T.amber, fontSize: '12px', textShadow: T.aGlow, whiteSpace: 'nowrap' }}>ROOT@JOBIE:~$</span>
            <input
              ref={inputRef}
              value={cmdInput}
              ...
              style={{
                flex: 1, background: 'transparent', border: 'none', color: T.green, fontFamily: FONT,
                fontSize: '12px', outline: 'none', textShadow: T.gGlow,
              }}
              spellCheck={false}
              autoComplete="off"
            />
            <span className={isDark ? 'term-blink' : ''} style={{ color: T.green, fontSize: '12px' }}>█</span>
          </div>
        </div>
